...
def job():
    ...
    fileimg = Path(os.getenv("HOME")+'/Pictures/Wallpapers/'+imgName[countspl])
    PathImg = os.getenv("HOME")+'/Pictures/Wallpapers/'+imgName[countspl]
    #print(fileimg)
    if fileimg.is_file():
        #os.system("/usr/bin/gsettings set org.gnome.desktop.background picture-uri " + os.getcwd() + '/' + imgName[countspl])
        os.system("/usr/bin/gsettings set org.gnome.desktop.background picture-uri " + PathImg)
    else:
        urllib.request.urlretrieve(imgLink, PathImg)
        #print(PathImg)
	#print(fileimg)
        os.system("/usr/bin/gsettings set org.gnome.desktop.background picture-uri "+PathImg)
